...
class MaintenanceManagerTest {
    ...
    @Test
    fun `getOperationPrediction with timeout only returns date prediction`() {
        ...
        val result = maintenanceManager.getOperationPrediction(opType, lastLog, null)
    ...
    @Test
    fun `getOperationPrediction with interval and trend returns usage prediction`() {
        ...
        val result = maintenanceManager.getOperationPrediction(opType, lastLog, 1.0)
    ...
    @Test
    fun `getOperationPrediction with both returns earliest date`() {
    ...
    @Test
    fun `getOperationPrediction with negative or zero trend ignores usage`() {
        val lastLog = MaintenanceLog(id = 1, equipmentId = 1, operationTypeId = 1, date = 0L, accumulatedValue = 100.0)
        val opType = OperationType(id = 1, description = "OT", intervalValue = 50.0)
        
        assertNull(maintenanceManager.getOperationPrediction(opType, lastLog, 0.0))
        assertNull(maintenanceManager.getOperationPrediction(opType, lastLog, -1.0))
    }
}
